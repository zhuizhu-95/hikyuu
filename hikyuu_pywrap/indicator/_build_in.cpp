...
:param bool ignore_context: 是否忽略上下文。忽略时，强制使用 query, market, stk_type 参数。)");
    ...
    m.def("ALIGN", py::overload_cast<const Indicator&, const Indicator&>(ALIGN), py::arg("ind"),
          py::arg("ref"));
    m.def("ALIGN", py::overload_cast<const Indicator&, const KData&>(ALIGN), py::arg("ind"),
          py::arg("ref"));
    m.def(
      "ALIGN",
      [](const py::list& ref) {
...
:retype: Indicator)");
    ...
    m.def("AMA", py::overload_cast<int, int, int>(AMA), py::arg("n") = 10, py::arg("fase_n") = 2,
          py::arg("slow_n") = 30);
    m.def(
      "AMA", py::overload_cast<const Indicator&, int, int, int>(AMA), py::arg("ind"),
      py::arg("n") = 10, py::arg("fase_n") = 2, py::arg("slow_n") = 30,
      R"(佩里.J 考夫曼（Perry J.Kaufman）自适应移动平均，参见《精明交易者》（2006年 广东经济出版社）

:param Indicator ind: 待计算的数据
:param int n: 计算均值的周期窗口，必须为大于2的整数，默认为10天
:param int fast_n: 对应快速周期N，默认为2
:param int slow_n: 对应慢速EMA线的N值，默认为30，不过当超过60左右该指标会收敛不会有太大的影响)");

    m.def("ASIN", py::overload_cast<void>(ASIN));
    m.def("ASIN", py::overload_cast<price_t>(ASIN));
    m.def("ASIN", py::overload_cast<const Indicator&>(ASIN), "反正弦值");

    m.def("ATAN", py::overload_cast<void>(ATAN));
    m.def("ATAN", py::overload_cast<price_t>(ATAN));
    m.def("ATAN", py::overload_cast<const Indicator&>(ATAN), "反正切值");
...
:param int n: 计算均值的周期窗口，必须为大于1的整数)");
    ...
    m.def("AVEDEV", AVEDEV, py::arg("ind"), py::arg("n") = 22,
          R"(平均绝对偏差，求X的N日平均绝对偏差

:param Indicator ind: 待计算的数据
:param int n 时间窗口)");

    m.def("BACKSET", py::overload_cast<int>(BACKSET), py::arg("n") = 2);
    m.def("BACKSET", py::overload_cast<const Indicator&, int>(BACKSET), py::arg("ind"),
          py::arg("n") = 2, R"(向前赋值将当前位置到若干周期前的数据设为1。
...
:param int n: N周期)");
    ...
    m.def("BARSCOUNT", py::overload_cast<const Indicator&>(BARSCOUNT), py::arg("ind"),
          R"(有效值周期数, 求总的周期数。
...
:param Indicator data: 输入数据)");
    ...
    m.def("BARSLAST", py::overload_cast<const Indicator&>(BARSLAST), py::arg("ind"),
          R"(上一次条件成立位置 上一次条件成立到当前的周期数。
...
:param Indicator ind: 输入数据)");
    ...
    m.def("BARSSINCE", py::overload_cast<const Indicator&>(BARSSINCE), py::arg("ind"),
          R"(第一个条件成立位置到当前的周期数。
...
:param Indicator ind: 输入数据)");
    ...
    m.def("BETWEEN",
          ...
          py::arg("a"), py::arg("b"), py::arg("c"), R"(介于(介于两个数之间)
...
:param c: C)");
    ...
    m.def("CEILING", py::overload_cast<price_t>(CEILING));
    m.def("CEILING", py::overload_cast<const Indicator&>(CEILING), py::arg("ind"),
          R"(向上舍入 (向数值增大方向舍入)
...
例如：CEILING(12.3)求得13；CEILING(-3.5)求得-3)");
    ...
    m.def("COS", py::overload_cast<const Indicator&>(COS), py::arg("ind"), R"(余弦值)");

    m.def("COST", py::overload_cast<double>(COST), py::arg("x") = 10.0);
    m.def("COST", py::overload_cast<const KData&, double>(COST), py::arg("kdata"),
          py::arg("x") = 10.0, R"(成本分布
...
:param float x: x%获利价格, 0~100)");
    ...
    m.def("COUNT", py::overload_cast<const Indicator&, int>(COUNT), py::arg("ind"),
          py::arg("n") = 20, R"(统计满足条件的周期数。
    
用法：COUNT(X,N),统计N周期中满足X条件的周期数,若N=0则从第一个有效值开始。
例如：COUNT(CLOSE>OPEN,20)表示统计20周期内收阳的周期数
    
:param Indicator ind: 条件
:param int n: 周期)");
...
:param y: 变量或常量，判断交叉的第二条线)");
    ...
    m.def("CVAL", py::overload_cast<double, size_t>(CVAL), py::arg("value"),
          py::arg("discard") = 0);
    m.def(
      "CVAL", py::overload_cast<const Indicator&, double, int>(CVAL), py::arg("data"),
      py::arg("value") = 0.0, py::arg("discard") = 0,
      R"(data 为 Indicator 实例，创建和 data 等长的常量指标，其值和为value，抛弃长度discard和data一样
...
:param int discard: 抛弃数量)");
    ...
    m.def(
      ...
      py::arg("query") = KQueryByIndex(-100), py::arg("market") = "SH",
      py::arg("stk_type") = StockType::A, py::arg("ignore_context") = false,
      R"(下跌家数。当存在指定上下文且 ignore_context 为 false 时，将忽略 query, market, stk_type 参数。

:param Query query: 查询条件
:param str market: 所属市场，等于 "" 时，获取所有市场
:param int stk_type: 证券类型, 等于 StockType.INVALID 时，获取所有类型证券
:param bool ignore_context: 是否忽略上下文。忽略时，强制使用 query, market, stk_type 参数。)");

    m.def("DEVSQ", py::overload_cast<int>(DEVSQ), py::arg("n") = 10);
    m.def("DEVSQ", py::overload_cast<const Indicator&, int>(DEVSQ), py::arg("ind"),
          py::arg("n") = 10, R"(数据偏差平方和，求X的N日数据偏差平方和

:param Indicator data: 输入数据
:param int n: 时间窗口)");

    m.def("DIFF", py::overload_cast<void>(DIFF));
    m.def("DIFF", py::overload_cast<const Indicator&>(DIFF), py::arg("ind"),
          R"(差分指标，即data[i] - data[i-1]

:param Indicator ind: 输入数据)");

    m.def("DMA", DMA, py::arg("ind"), py::arg("a"), R"(动态移动平均
...
:param Indicator a: 动态系数)");
    ...
    m.def("DOWNNDAY", DOWNNDAY, py::arg("ind"), py::arg("n") = 3,
          R"(连跌周期数, DOWNNDAY(CLOSE,M)表示连涨M个周期

:param Indicator ind: 输入数据
:param int n: 时间窗口)");

    m.def("DROPNA", py::overload_cast<void>(DROPNA));
    m.def("DROPNA", py::overload_cast<const Indicator&>(DROPNA), R"(删除 nan 值)");

    m.def("EMA", py::overload_cast<int>(EMA), py::arg("n") = 22);
    m.def("EMA", py::overload_cast<const Indicator&, int>(EMA), py::arg("ind"), py::arg("n") = 22,
          R"(指数移动平均线(Exponential Moving Average)

:param Indicator ind: 输入数据
:param int n: 计算均值的周期窗口，必须为大于0的整数)");

    m.def("EVERY", py::overload_cast<int>(EVERY), py::arg("n") = 20);
    m.def("EVERY", py::overload_cast<const Indicator&, int>(EVERY), py::arg("ind"),
          py::arg("n") = 20, R"(一直存在
...
:param int n: 计算均值的周期窗口，必须为大于0的整数 )");
    ...
    m.def("EXP", py::overload_cast<const Indicator&>(EXP), R"(EXP(X)为e的X次幂)");

    m.def("FILTER", py::overload_cast<int>(FILTER), py::arg("n") = 5);
    m.def("FILTER", py::overload_cast<const Indicator&, int>(FILTER), py::arg("ind"),
          py::arg("n") = 5, R"(信号过滤, 过滤连续出现的信号。
...
例如：FLOOR(12.3)求得12)");
    ...
    m.def("HHV", py::overload_cast<const Indicator&, int>(HHV), py::arg("ind"), py::arg("n") = 20,
          R"(N日内最高价, N=0则从第一个有效值开始。
...
:param int n: n周期)");
    ...
    m.def("LIUTONGPAN", py::overload_cast<const KData&>(LIUTONGPAN), R"(获取流通盘
   
:param KData kdata: k线数据
:rtype: Indicator)");

    m.def("CAPITAL", py::overload_cast<void>(LIUTONGPAN));
    m.def("CAPITAL", py::overload_cast<const KData&>(LIUTONGPAN), R"(获取流通盘
   
:param KData kdata: k线数据
:rtype: Indicator)");

    m.def("LLV", py::overload_cast<int>(LLV), py::arg("n") = 20);
    m.def("LLV", py::overload_cast<const Indicator&, int>(LLV), py::arg("ind"), py::arg("n") = 20,
          R"(N日内最低价, N=0则从第一个有效值开始。

:param data: 输入数据
:param int n: N日时间窗口)");

    m.def("LLVBARS", py::overload_cast<int>(LLVBARS), py::arg("n") = 20);
    m.def("LLVBARS", py::overload_cast<const Indicator&, int>(LLVBARS), py::arg("ind"),
          py::arg("n") = 20, R"(上一低点位置 求上一低点到当前的周期数。
...
:param int n: 周期窗口)");
    ...
    m.def("LN", py::overload_cast<const Indicator&>(LN), "求自然对数, LN(X)以e为底的对数");

    m.def("LOG", py::overload_cast<void>(LOG));
    m.def("LOG", py::overload_cast<price_t>(LOG));
    m.def("LOG", py::overload_cast<const Indicator&>(LOG), "以10为底的对数");
